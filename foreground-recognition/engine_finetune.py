...
def train_one_epoch(model: LLaMA_adapter,
    ...
    for data_iter_step, (examples, labels, example_mask, imgs_clip) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader) + epoch, args)

        imgs_clip = imgs_clip.to(device, non_blocking=True)
        # imgs_dino = imgs_dino.to(device, non_blocking=True)

        with torch.cuda.amp.autocast():
             c_loss, m_loss = model(examples, labels, imgs_clip)  # input2(question+answer), answer, image
