...
class BERTTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], prompt=None):
        xq, xo = self.attention.forward(self.attention_norm(x), start_pos, freqs_cis, mask, prompt)
        h = x + xo
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        return out, xq
...
class BERTTransformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        layer_xq = []
        for layer in self.layers:
            h,xq = layer(h, start_pos, freqs_cis, mask)
            layer_xq.append(xq)
        h = self.norm(h)
        output = self.output(h)  # only compute last logits
        return output,layer_xq


class BERTTransformer_cross(nn.Module):
    def __init__(self, params: ModelArgs):
        super().__init__()
        self.params = params
        self.n_layers = params.n_layers
        ...
        self.layers = torch.nn.ModuleList()
        for layer_id in range(params.n_layers):
            self.layers.append(BERTTransformerBlock_cross(layer_id, params))
    ...
    def forward(self, shared_q: torch.Tensor, tokens: torch.Tensor, start_pos: int):
        _bsz, seqlen, channel = tokens.shape
        h = tokens
        self.freqs_cis = self.freqs_cis.to(h.device)
        freqs_cis = self.freqs_cis
        ...
        for layer in self.layers:
            xq = shared_q[count]
            count += 1
            h = layer(h, xq, start_pos, freqs_cis, mask)
        h = self.norm(h)
        output = self.output(h)  # only compute last logits
        return output


class BERTTransformerBlock_cross(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        super().__init__()
        self.n_heads = args.n_heads
        self.dim = args.dim
        self.head_dim = args.dim // args.n_heads
        self.attention = BERTAttention_cross(args)
        self.feed_forward = FeedForward(
            lora=False, bias=False, dim=args.dim, hidden_dim=4 * args.dim, multiple_of=args.multiple_of, args=args
        )
        self.layer_id = layer_id
    ...
    def forward(self, x: torch.Tensor, xq: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], prompt=None):

        h = x + self.attention.forward(xq, self.attention_norm(x), start_pos, freqs_cis, mask, prompt)
        out = h + self.feed_forward.forward(self.ffn_norm(h))
        return out
...
class BERTAttention_cross(nn.Module):
    ...
    def forward(self, shared_q: torch.Tensor, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], prompt=None):
        bsz, seqlen, _ = x.shape
        xk, xv =  self.wk(x), self.wv(x)

        # xq = xq.view(bsz, seqlen, self.n_local_heads, self.head_dim)
        xq = shared_q.transpose(1,2)
        xk = xk.view(bsz, seqlen, self.n_local_heads, self.head_dim)
        xv = xv.view(bsz, seqlen, self.n_local_heads, self.head_dim)
