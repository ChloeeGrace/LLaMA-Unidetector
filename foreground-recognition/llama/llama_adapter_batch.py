...
class LLaMA_adapter(nn.Module):
    ...
    def generate(
            ...
            if temperature > 0:
                ...
                if prev_pos == 0:
                    ...
                    for bsz_idx in range(bsz):
                        ...
                        check_id = tokens[:, prev_pos:cur_pos].tolist()[bsz_idx]
                        decode_prev_toekn = self.tokenizer.decode(check_id)
                        # --------------------------------------------------------check next----------------
                        ...
                        if '[' in decode_next_token:
                            ...
                            if s_flag[bsz_idx] == False:
                                pass
                            elif ']' not in decode_next_token and s_flag[bsz_idx] == True:  # ']'
                                ...
                                if decode_next_token == "'" or decode_next_token == "," or decode_next_token == "'," or decode_next_token == "" or decode_next_token == ", " or decode_next_token == "', " or decode_next_token == ".":
                                    pass
                                elif decode_prev_toekn == ".":
                                    pass
                                else:
                                    conf[bsz_idx] += torch.max(torch.softmax(logits[bsz_idx], dim=-1)).tolist()
                                    count[bsz_idx] += 1
                            else:
                                conf[bsz_idx] = conf[bsz_idx] / count[bsz_idx]
                                if bsz_idx not in conf_dict:
                                    conf_dict[bsz_idx] = [conf[bsz_idx]]
                                else:
                                    conf_dict[bsz_idx].append(conf[bsz_idx])

                                # if conf == 0:
                                #     if bsz_idx not in conf_dict:
                                #         conf_dict[bsz_idx] = [conf]
                                #     else:
                                #         pass
                                # else:
                                #     conf = conf / count
        ...
        if temperature == 0.0:
            print(conf_dict)
            for bsz_idx in range(bsz):
                result = decoded[bsz_idx]
                if bsz_idx not in conf_dict:
                    print("WTF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    continue

                det_num = len(conf_dict[bsz_idx])
                insert_list = []
                flag = False
                ...
                for i in range(len(insert_list)):
                    ...
                    sub_result = result[start_id:insert_id] + " %.2f,"%(conf_dict[bsz_idx][i])
...
def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts', max_seq_len=512,
    ...
    model = LLaMA_adapter(
        ...
        max_seq_len=max_seq_len, max_batch_size=64,
        clip_model='ViT-L/14@336px',
        v_embed_dim=1024, v_depth=16,
        v_num_heads=16, v_mlp_ratio=4.0,
